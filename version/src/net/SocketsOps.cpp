...
int createNonblockingOrDie(sa_family_t family){
    //创建套接字,TCP流,非阻塞,CLSOE ON EXEC，fork后关闭这个无用的fd
...
void shutdownWrite(int sockfd){
    if(::shutdown(sockfd, SHUT_WR) < 0){
        LOG_SYSFATAL << "sockets::shiutdownWrite";
    }
}
...
bool isSelfConnct(int sockfd){
    ...
    if(localaddr.sin6_family == AF_INET){ //若是IPV4,需要把sockaddr_in6转换成sockaddr_in进行判断
